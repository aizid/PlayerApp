...
public class RestClient {
    ...
    private func responseDecodeableProcessor<T: Decodable>(data: DataResponse<T, AFError>) -> ApiResult<T>? {
        ...
        switch data.result {
            ...
            case .failure(let afError): do {
                ...
                return .error(ApiError(status: data.response?.statusCode ?? GeneralErrorCode,
    ...
    public func delete<T: Decodable, U: Encodable>(url: String, param: U?, responseType: T.Type, onComplete: @escaping (ApiResult<T>) -> (), timeout: Double = 60) -> DataRequest {
        let param = objectToDict(data: param)
        return request(url: url, method: .delete, param: param, responseType: responseType, onComplete: onComplete, timeout: timeout)
    }
    
    public func delete<T: Decodable>(url: String, responseType: T.Type, onComplete: @escaping (ApiResult<T>) -> (), timeout: Double = 60, encoding: ParameterEncoding = URLEncoding.queryString) -> DataRequest {
        return request(url: url, method: .delete, param: nil, responseType: responseType, onComplete: onComplete, timeout: timeout)
    }
